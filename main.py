...
@app.on_message(filters.regex("ساخت پست"))
async def story (client, message):
    await client.send_message(chat_id=message.chat.id , text="لطفا عکس های پست را ارسال کنید")

@app.on_message(filters.document)
async def download_story_pic (client, message):
    last_message = await app.get_messages(message.chat.id, int(message.id)-1)
    if last_message.text == "لطفا عکس های پست را ارسال کنید":
        await client.download_media(message)
